...
export default {
	...
	methods: {
		...
		async register(event) {
      		await axios.post(`/events/register`, { id: event._id });
			this.loadAll()
			this.event.ticketsCount -= 1;
		}
	},
			...
			<div class="row row-cols-2 justify-content-center">
				...
				<div v-if="true" class="col">
					...
					<div class="input-group mb-3">
						<select @change="search" class="form-select form-select-lg" v-model="month">
					...
					<!-- Карточки событий -->
					<ul class="list-group">
						<li @click="select(index)" class="list-group-item" v-for="(item, index) in events">
				...
				<div v-if="event" class="col show-event">
					<div class="card mb-3">
						<div class="card-body">
							<h2 class="card-title">
								{{ event.title }}
						...
						<img :src="'../assets/' + event.image" class="card-img-bottom" />
					</div>
				</div>
			</div>
		</div>
